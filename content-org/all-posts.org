...
あとは生成された PDF を「長辺とじ」で両面印刷すれば、中綴じ可能な小冊子用プリントが出力される。

*** 背景

息子が誕生日に [[https://amzn.to/4oDjvg4][完全爆弾解除マニュアル:Keep Talking and Nobody Explodes]] を欲しがった。
このゲームは、爆弾処理担当とマニュアル担当に分かれて協力するゲームで、マニュアル担当には紙のマニュアルが必要不可欠だ（雰囲気を出すためにも）。

せっかくなので、それらしく、かつコンパクトにする為に小冊子印刷 (ブックレット印刷) をしたかった。

*** 試行錯誤の記録

「Ubuntu 小冊子印刷」などで検索すると、いくつかの方法が見つかったが、簡単にはいかなかった。

**** 標準のプリンタ設定

[[https://help.ubuntu.com/stable/ubuntu-help/printing-booklet-duplex.html.ja][公式ヘルプ]] には、「ページ番号を 4, 1, 2, 3 の順に入力して……」といった気の遠くなるような手動指定方法が記載されていた。ページ数が多いマニュアルでこれをやるのは現実的ではない。

**** 既存ツール (失敗)

1. **mkbookpdf**: Rust 製のツール。試してみたが、以下のようなエラーで panic して動かなかった。
   #+begin_example
...
2. **boomaga**: 仮想プリンタツール。PPA を追加しようとしたが、Ubuntu 24.04 (Noble) 向けのリリースが見つからず 404 エラーとなった。

*** 解決策: pdfbook2

最終的にたどり着いたのが [[https://github.com/jenom/pdfbook2][pdfbook2]] 。
Python 製だが、Ubuntu では `texlive-extra-utils` パッケージの一部として提供されていることが分かった。
...
TexLive 関連の依存パッケージが大量にインストールされるため、環境によっては時間がかかる（手元では 2 分弱）。
...
これだけで、 =KeepTalkingAndNobodyExplodes-BombDefusalManual-v1-ja-book.pdf= というファイルが生成される。このファイルを PDF ビューワで開き、プリンタの設定で「両面印刷（長辺とじ）」を指定して印刷すれば完了だ。
