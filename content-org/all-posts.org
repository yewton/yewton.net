...
** DONE 完全爆弾解除マニュアルのマニュアルPDFをいい感じに印刷する、または Ubuntu で小冊子印刷をする方法
CLOSED: [2025-12-07 日 22:30]
:PROPERTIES:
:EXPORT_FILE_NAME:   index
:EXPORT_DATE:        2025-12-07
:DIR:                images/2025-12-07_ubuntu-booket-printing-pdfbook2
:EXPORT_HUGO_BUNDLE: 2025-12-07_ubuntu-booket-printing-pdfbook2
:EXPORT_HUGO_SLUG:   ubuntu-booket-printing-pdfbook2
:EXPORT_HUGO_TAGS: ubuntu pdf printing booklet
:END:
...
Ubuntu 24.04 (Noble) で PDF を小冊子 (Booklet) 印刷用に加工するには、 =pdfbook2= が便利です。
...
#+begin_src shell
  ...
  # 変換
  pdfbook2 input.pdf
  # -> input-book-le.pdf が生成される
#+end_src

あとは生成された PDF を「長辺とじ」で両面印刷すれば、中綴じ可能な小冊子用プリントが出力されます。

*** 背景

息子が誕生日に [[https://amzn.to/4oDjvg4][完全爆弾解除マニュアル:Keep Talking and Nobody Explodes]] を欲しがりました。
このゲームは、爆弾処理担当とマニュアル担当に分かれて協力するゲームで、マニュアル担当には紙のマニュアルが必要不可欠です（雰囲気を出すためにも）。

せっかくなので、それらしく、かつコンパクトにする為に小冊子印刷 (ブックレット印刷) をしたくなりました。

*** 試行錯誤の記録

「Ubuntu 小冊子印刷」などで検索すると、いくつかの方法が見つかりましたが、簡単にはいきませんでした。

**** 標準のプリンタ設定

[[https://help.ubuntu.com/stable/ubuntu-help/printing-booklet-duplex.html.ja][公式ヘルプ]] には、「ページ番号を 4, 1, 2, 3 の順に入力して……」といった気の遠くなるような手動指定方法が記載されていました。ページ数が多いマニュアルでこれをやるのは現実的ではありません。

**** 既存ツール (失敗)

1. **mkbookpdf**: Rust 製のツール。試してみましたが、以下のようなエラーで panic して動きませんでした。
   #+begin_example
...
2. **boomaga**: 仮想プリンタツール。PPA を追加しようとしましたが、Ubuntu 24.04 (Noble) 向けのリリースが見つからず 404 エラーとなりました。

*** 解決策: pdfbook2

最終的にたどり着いたのが [[https://github.com/jenom/pdfbook2][pdfbook2]] です。
Python 製ですが、Ubuntu では `texlive-extra-utils` パッケージの一部として提供されていることが分かりました。
...
これを実行すると、 =input-book-le.pdf= というファイルが生成されます。
=pdfbook2= は、入力された PDF のお隣同士のページを 1 枚の紙に並べる面付け (Imposition) 処理を行ってくれます (A4 用紙に A5 サイズ 2 ページを印刷するイメージです)。

このファイルを PDF ビューワで開き、プリンタの設定で **「両面印刷（長辺とじ: Long-edge binding）」** を指定して印刷すれば完了です。
プリンタや紙の向きによっては「短辺とじ」にする必要がある場合もあるため、印刷プレビューでページの向きを確認することをお勧めします。

**** 応用: 余白の調整

もし印刷結果の余白が気になる場合は、 =--inner-margin= などのオプションで調整が可能です。

#+begin_src shell
  # コンテンツを用紙の端に寄せる例
  pdfbook2 --inner-margin=0 --outer-margin=0 --top-margin=0 --bottom-margin=0 input.pdf
#+end_src

** DONE Ubuntu で Bluetooth マウスが使えない時にやるといいかもしれないこと
CLOSED: [2022-05-29 日 23:28]
:PROPERTIES:
:EXPORT_FILE_NAME:   index
:EXPORT_DATE:        2022-05-29
:DIR:                images/2022-05-29_ubuntu-bluetooth-troubleshoot
:EXPORT_HUGO_BUNDLE: 2022-05-29_ubuntu-bluetooth-troubleshoot
:EXPORT_HUGO_SLUG:   ubuntu-bluetooth-troubleshoot
:EXPORT_HUGO_TAGS: ubuntu bluetooth mouse
:END:
...
[[/2021/10/11/dell-latitude-3420-byo-ubuntu/][以前購入した Dell Latitude 3420]] ( Xubuntu 入り )をクラムシェルで使うときに、マウスが入り用になった。

元々 [[https://amzn.to/3wZm0kh][ロジクール M337]] を持っていたので、これを使えばいいと思っていたのだけれど、何故か検出されない。
具体的には =blueman-manager= から、マウスが見えない。

検索してみると [[https://ubuntuforums.org/showthread.php?t=2390542][[SOLVED] Bluetooth Mouse Won't Connect after Reboot - Ubuntu 18.04 LTS]] が見つかった。

理屈は分からないのだけれど、 [[http://www.bluez.org/][BlueZ]] の =bluetoothctl= を使わないと接続出来ない機器があるらしい。
...
** DONE Dell Latitude 3420 の SSD をより大容量の SSD に Clonezilla でクローンし換装する
CLOSED: [2025-01-06 月 01:05]
:PROPERTIES:
:EXPORT_FILE_NAME:   index
:EXPORT_DATE:        2025-01-05
:DIR:                images/2025-01-05_dell-latitude-3420-storage
:EXPORT_HUGO_BUNDLE: 2025-01-05_dell-latitude-3420-storage
:EXPORT_HUGO_SLUG:   dell-latitude-3420-storage
:EXPORT_HUGO_TAGS: dell latitude clonezilla
:END:
...
[[/2022/02/19/dell-latitude-3420-memory/][以前の記事]] では [[https://japancatalog.dell.com/pd/latitude-3420.html][Dell Latitude 3420]] にメモリを増設したが、今回は SSD を換装した。
...
今回購入した SSD は、 [[https://amzn.to/3DHMFZP][キオクシア ( KIOXIA ) の EXCERIA PRO NVMe™ M.2 Type 2280-S2-M SSD 1TB]] 。
[[https://www.dell.com/support/kbdoc/en-us/000185381/dell-latitude-3420-notebook-teardown-removal-guide-for-customer-replaceable-units-crus?lwp=rt][Dell Latitude 3420 Notebook Teardown removal guide for customer replaceable units (CRUS) | Dell US]] に
M.2 SSD 2280 card slot の記載があるので問題ないハズ。

この SSD を [[https://amzn.to/40jZ5jc][ロジテック ( Logitec ) の SSD ケース LGB-PNV02UC]] にセットして、データのクローンを行った。

メモリは [[https://amzn.to/405kHhX][Crucial 16GB DDR4-3200 SODIMM D4N3200CM-16GQ]] を購入。

*** Clonezilla による SSD クローン

クローンには [[https://clonezilla.org/][Clonezilla]] を利用した。

まずは [[https://clonezilla.org///liveusb.php][Clonezilla Live on USB]] に従って Live USB を作成する。

今回はたまたま余っていた [[https://amzn.to/3PJ2Fh3][エレコム 外付けポータブルSSD ESD-ED0120GBK 120GB]] を利用したが、  500 MiB あれば何でもよいハズ。

手順に従い、 FAT でフォーマットした後に =unzip= で配置して完成。

この Live USB と、クローン先の SSD を収めたケースを接続した上で、 [[https://www.dell.com/support/manuals/ja-jp/latitude-14-3420-laptop/latitude_3420_cyborg_sm/%E3%83%96%E3%83%BC%E3%83%88%E3%83%A1%E3%83%8B%E3%83%A5%E3%83%BC?guid=guid-493feb0b-5f68-470b-83ef-b8a975eb6e9d&lang=ja-jp#:~:text=%E3%82%B7%E3%82%B9%E3%83%86%E3%83%A0%E3%81%AE%E6%9C%89%E5%8A%B9%E3%81%AA%E8%B5%B7%E5%8B%95,%E3%82%92%E6%8A%BC%E3%81%97%E3%81%BE%E3%81%99%E3%80%82][Latitude 3420 サービス マニュアル | Dell 日本]] に従い、起動後 Dell のロゴが表示されたときに =<F12>= を押してブートメニューに入り、 Clonezilla の Live USB を選択する。

その後は [[https://clonezilla.org///fine-print-live-doc.php?path=clonezilla-live/doc/03_Disk_to_disk_clone][Clonezilla Live Doc の Disk to disk clone のページ]] に従っていけばよい。

#+caption: Start Clonezilla を選択する
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-05_23-51-16_screenshot.png]]

#+caption: device-device を選択する
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-05_23-52-43_screenshot.png]]


元々単一のパーティションにしていたので、今回は [[https://clonezilla.org/clonezilla-live/doc/03_Disk_to_disk_clone/advanced/05-advanced-param.php][Clonezilla - Advanced Mode]] に従いパーティションも含めてよしなに拡張するオプションを有効化する為、 =Expert= を選択する。

#+caption: Expert を選択する
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-05_23-57-51_screenshot.png]]


#+caption: disk_to_local_disk を選択する
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-05_23-58-30_screenshot.png]]


#+caption: クローン元のディスクを選択する
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-04-16_screenshot.png]]

#+caption: クローン先のディスクを選択する(元のデータは全消去される)
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-10-00_screenshot.png]]


#+caption: advanced extra parameters はデフォルトのまま
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-11-37_screenshot.png]]


#+caption: ファイルシステムのチェックはスキップ(デフォルト)
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-12-26_screenshot.png]]


#+caption: ここで -k1 Create partition table proportionally を選択する
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-13-22_screenshot.png]]


#+caption: 終了後の挙動はその時に選択することにして、進める
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-14-18_screenshot.png]]

#+caption: とても熱心に確認してくれるので、覚悟を決めて y する
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-15-15_screenshot.png]]
...
[[/2022/02/19/dell-latitude-3420-memory/][以前の記事]] 同様にサービスモードに入り、ベースカバーを外して作業していく
( 今回気づいた点は以前の記事側に注記を入れた )。

元々は M.2 type2242 が使われていた為、以下の部分でネジ止めされていた。

#+caption: 元々ネジ止めされていた部分
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-32-06_screenshot.png]]

換装するのは M.2 type2280 の為、この部品自体を移動する必要があった。

#+caption: ネジを外して固定金具自体を移動する
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-36-59_screenshot.png]]
...
取り付けは [[/2022/02/19/dell-latitude-3420-memory/][以前の記事]] 同様だが、今回は元々入っていた 8 GiB メモリと交換する必要がある為、まず取り外す必要がある。

以下の固定金具部分を外側に押し広げるような感じにすると、バインッと外れた。

#+caption: 固定金具。右側は隠れてしまっているが同じ箇所にある。
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-42-26_screenshot.png]]

外したあとは、 [[/2022/02/19/dell-latitude-3420-memory/][以前の記事]] 同様に新しいメモリをセットして完了。

#+caption: 追加分。こちら側は向きを逆にする必要があった。
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_00-50-01_screenshot.png]]
...
#+caption: Memory: 31.10 GiB 、 Disk (/): 907.06 GiB となっている
#+attr_org: :width 300px
[[file:images/2025-01-05_dell-latitude-3420-storage/2025-01-06_01-02-47_screenshot.png]]
...
:EXPORT_HUGO_TAGS: dell latitude
:END:
...
:EXPORT_HUGO_TAGS: dell xubuntu ubuntu xfce latitude
...
** DONE Elastic Common Schema と OpenTelemetry Semantic Convention を通して Structured Logging について学んだ
CLOSED: [2025-01-08 水 23:35]
:PROPERTIES:
:EXPORT_FILE_NAME:   index
:EXPORT_DATE:        2025-01-03
:DIR:                images/2025-01-03_structured-logging-ecs-otel-semconv
:EXPORT_HUGO_BUNDLE: 2025-01-03_structured-logging-ecs-otel-semconv
:EXPORT_HUGO_SLUG:   structured-logging-ecs-otel-semconv
:EXPORT_HUGO_TAGS: "Spring Boot" "Structured logging" "Elastic Common Schema" "OpenTelemetry Semantic Convention"
:END:
...
[[https://spring.io/blog/2024/08/23/structured-logging-in-spring-boot-3-4][Structured logging in Spring Boot 3.4]] で、 Structured logging のサポートが発表されていた。

Spring Boot として提供される機能は分かったが、そもそもの Structured logging、構造化ロギングについて理解出来ていなかったので、調べることにした。

とっかかりとして、 Spring Boot がサポートする Structured logging フォーマットとしていの一番に挙げられている [[https://www.elastic.co/guide/en/ecs/8.11/ecs-reference.html][Elastic Common Schema (ECS)]] を通して調べたことをまとめる。

*** Structured logging とは

[[https://www.elastic.co/guide/en/ecs/current/ecs-getting-started.html][Getting Started | Elastic Common Schema (ECS) Reference [8.16] | Elastic]] には以下のようにある:
...
Note that when adopting an Elastic solution, such as [[https://www.elastic.co/observability][Observability]] or [[https://www.elastic.co/security][Security]], all events will map to ECS out of the box. Elastic provides an extensive set of [[https://www.elastic.co/security][integrations]] to simplify ingesting your data sources.
#+end_quote
...
Elastic スタックと共に語られる事の多い [[https://grafana.com/][Grafana]] の分散トレーシングソリューションである [[https://grafana.com/docs/tempo/latest/traceql/trace-structure/][Tempo のドキュメント]] には、vendor-agnositc な Observability フレームワークである [[https://opentelemetry.io/][OpenTelemetry]] が提供する [[https://opentelemetry.io/docs/specs/semconv/][OpenTelemetry Semantic Conventions 1.29.0 | OpenTelemetry]] について言及されている。
...
両者の関係は [[https://github.com/open-telemetry/oteps/blob/main/text/0199-support-elastic-common-schema-in-opentelemetry.md#scope-and-overlap-of-ecs-and-otel-semconv][oteps/text/0199-support-elastic-common-schema-in-opentelemetry.md at main · open-telemetry/oteps]] の図を見ると分かり易い。

提供価値被ってるし、統合した方がよいよね、ということで Elastic と OpenTelemetry からそれぞれ以下のアナウンスがあった:

- [[https://opentelemetry.io/blog/2023/ecs-otel-semconv-convergence/][Announcing the Elastic Common Schema (ECS) and OpenTelemetry Semantic Convention Convergence | OpenTelemetry]]
- [[https://www.elastic.co/blog/ecs-elastic-common-schema-otel-opentelemetry-announcement][Elastic contributes Elastic Common Schema (ECS) to OpenTelemetry project, helping accelerate adoption of OTel-based observability and security | Elastic Blog]]
...
[[https://opentelemetry.io/community/roadmap/][OpenTelemetry Project Roadmap | OpenTelemetry]] には Further Stabilizing Semantic Conventions が most important major initiatives として挙げられているので、統合に向けて確実に進んでいるようではあるが、現在のところは安定して利用出来る具体的な成果物はまだ無いようだ。
...
先に挙げた [[https://spring.io/blog/2024/08/23/structured-logging-in-spring-boot-3-4][Structured logging in Spring Boot 3.4]] に拠れば、 =logging.structured.format.console=ecs= と設定するだけで以下のようなログが出力されるようになる:
...
- [[https://www.elastic.co/guide/en/ecs/current/ecs-base.html#field-timestamp][@timestamp]]
- [[https://www.elastic.co/guide/en/ecs/current/ecs-log.html#field-log-level][log.level]]
- [[https://www.elastic.co/guide/en/ecs/current/ecs-process.html#field-process-pid][process.pid]]
- [[https://www.elastic.co/guide/en/ecs/current/ecs-process.html#field-process-thread-name][process.thread.name]]
- [[https://www.elastic.co/guide/en/ecs/current/ecs-service.html#field-service-name][service.name]]
- [[https://www.elastic.co/guide/en/ecs/current/ecs-log.html#field-log-logger][log.logger]]
- [[https://www.elastic.co/guide/en/ecs/current/ecs-base.html#field-message][message]]

実は Elastic 提供の [[https://github.com/elastic/ecs-logging-java/tree/v1.6.0?tab=readme-ov-file][elastic/ecs-logging-java at v1.6.0]] というものもあるのだが、 Spring Boot はこれを使っているわけ *ではない* 。
[[https://github.com/spring-projects/spring-boot/blob/v3.4.1/spring-boot-project/spring-boot/src/main/java/org/springframework/boot/logging/logback/ElasticCommonSchemaStructuredLogFormatter.java#L52-L72][独自に実装している]] 。
...
[[https://github.com/elastic/ecs-logging-java/issues/49][Support structured logging with Logback · Issue #49 · elastic/ecs-logging-java]]
...
ECS と OTel SemConv 統合作業は今まさに進行中という感じで、ログに関しても [[https://github.com/open-telemetry/community#specification-sigs][open-telemetry/community: OpenTelemetry community content]] から辿れる [[https://docs.google.com/document/d/1BKjQWP32FXL9g1cGbyj7DMXV1Uq_RL8_78rWaMBhN0A/edit?tab=t.0][OpenTelemetry Log SIG - Google ドキュメント]] にて、 [[https://github.com/open-telemetry/semantic-conventions/issues/1651][Logs/Events guidance on what to put to attributes vs body · Issue #1651 · open-telemetry/semantic-conventions]] が挙げられていた事からも、活発に議論がされているようす。
...
[[https://github.com/spring-projects/spring-boot/issues/34132][Micrometer Tracing with OTEL Bridge does not honour Semantic Conventions · Issue #34132 · spring-projects/spring-boot]]
[[https://github.com/micrometer-metrics/tracing/issues/929][Implement OTel HTTP semantic conventions · Issue #929 · micrometer-metrics/tracing]]

Java の artifact がまだ alpha 状態であることが理由だそう。
確かに、 stable な spec に対する artifact でも [[https://search.maven.org/artifact/io.opentelemetry.semconv/opentelemetry-semconv/1.29.0-alpha/jar?eh][1.29.0-alpha]] のようなバージョニングになっている。
...
これは [[https://opentelemetry.io/docs/specs/otlp/][OpenTelemetry Protocol (OTLP)]] に従って、 gRPC や HTTP などで [[https://opentelemetry.io/docs/specs/otel/][OpenTelemetry Specification]] に則ったデータを送信する機能で、 OTel SemConv のような標準規格というよりは OpenTelemetry という実装に特化した機能。

これで利用される [[https://opentelemetry.io/docs/specs/otel/logs/data-model/][Logs Data Model]] は OTel SemConv に則って定義されていたりするので、無関係というわけではない。
...
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/code/#code-filepath][code.filepath]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/code/#code-function][code.function]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/code/#code-lineno][code.lineno]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/code/#code-namespace][code.namespace]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/process/#process-pid][process.pid]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/thread/#thread-name][thread.name]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/service/#service-name][service.name]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/telemetry/#telemetry-sdk-language][telemetry.sdk.language]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/telemetry/#telemetry-sdk-name][telemetry.sdk.name]]
- [[https://opentelemetry.io/docs/specs/semconv/attributes-registry/telemetry/#telemetry-sdk-version][telemetry.sdk.version]]
...
** DONE Rust で GBA 開発のとっかかり
CLOSED: [2024-05-12 日 00:02]
:PROPERTIES:
:EXPORT_FILE_NAME:   index
:EXPORT_DATE:        2024-05-11
:DIR:                images/2024-05-11_gba-dev-rust
:EXPORT_HUGO_BUNDLE: 2024-05-11_gba-dev-rust
:EXPORT_HUGO_SLUG:   gba-dev-rust
:EXPORT_HUGO_TAGS: gba rust agb
:END:
...
[[https://www.rust-lang.org/][Rust]] で出来ないかと検討していた。

当時  [[https://github.com/devkitPro/libgba][libgba]] を利用してつくったものは以下。これらを Rust で書き直すことが目標。

**** jumpingdroid

#+attr_org: :width 300px
#+caption: ドロイド君が跳ねたり銀の林檎に乗ったりする
[[file:images/2024-05-11_gba-dev-rust/2024-05-11_20-44-18_jumpingdroid_mb-1.png]]
...
#+attr_org: :width 300px
#+caption: ドロイド君が右へ右へと進んでいく。ボタン押下で色も変わる。
[[file:images/2024-05-11_gba-dev-rust/2024-05-11_20-57-33_jumpingdroid2_mb-1.png]]
...
--  [[https://www.coranac.com/tonc/text/hardware.htm][/Tonc: GBA Hardware/]]
#+end_quote
...
#+attr_org: :width 300px
#+caption: ドロイド君の左下部分の画像が =0x06010040= にある様子
[[file:images/2024-05-11_gba-dev-rust/2024-05-11_21-43-09_screenshot.png]]
...
#+attr_org: :width 300px
#+caption: 256×256 ピクセルの BG
[[file:images/2024-05-11_gba-dev-rust/2024-05-11_22-06-15_screenshot.png]]
...
#+attr_org: :width 300px
#+caption: =0x07000000= に格納されているドロイド君のスプライトデータ。参照先タイルは =0x06010180= にある。
[[file:images/2024-05-11_gba-dev-rust/2024-05-11_22-15-29_screenshot.png]]
...
今回利用する [[https://agbrs.dev/][agb]] というライブラリでは、 GBA でのゲーム開発に必要な機能をかなり抽象化して扱い易くしてくれている。

スプライトは [[https://www.aseprite.org/][Aseprite]] というピクセルアート・アニメーション作成ツールのフォーマットをそのまま利用出来る
( Aseprite は購入するか、自身でビルドする必要がある )。

#+attr_org: :width 300px
#+caption: Aseprite の編集画面
[[file:images/2024-05-11_gba-dev-rust/2024-05-11_23-00-28_screenshot.png]]

#+caption: コードからのスプライト用データ利用イメージ
#+begin_src rust
...
[[https://mgba.io/][mGBA]] で jumpingdroidr2 を実行している様子が以下。

BG1 のオフセットとタイルが更新される様子を併せて収録している。
ドロイド君の進行方向にステージを書き足していっている様子が観察出来る。

@@html:{{< video src="Screencast 2024-05-11 23:37:55.mp4" controls="yes" >}}@@
...
- https://github.com/agbrs/agb/tree/master/examples
  ...
  その他 GBA 開発関連の情報は [[https://gbadev.net/][Home | gbadev]] にまとまっている。

** DONE Spring Boot でプロファイルに応じて読み込む設定ファイルの設定値をテストする :spring:spring_boot:kotest:Java:Kotlin:
CLOSED: [2022-12-29 木 23:53]
:PROPERTIES:
:EXPORT_FILE_NAME:   index
:EXPORT_DATE:        2022-12-29
:DIR:                images/2022-12-29_spring-boot-config-test
:EXPORT_HUGO_BUNDLE: 2022-12-29_spring-boot-config-test
:EXPORT_HUGO_SLUG:   spring-boot-config-test
:END:

*** 参考

- [[https://www.baeldung.com/spring-boot-testing-configurationproperties][Testing Spring Boot @ConfigurationProperties | Baeldung]]
- [[https://nainaistar.hatenablog.com/entry/2021/03/30/120000][【2021】SpringBootでpropertiesやymlの設定ファイルが読み込めることのテストを書く - きり丸の技術日記]]


*** 本文

[[https://docs.spring.io/spring-boot/docs/3.0.1/reference/htmlsingle/#features.external-config][公式リファレンスドキュメント]] に記載されているように、 Spring Boot での外部設定ファイル読み込み方法は色々ある。

=application.(yml|properties)= と、 [[https://docs.spring.io/spring-boot/docs/3.0.1/reference/htmlsingle/#features.profiles][Profile]] 毎に =application-production.(yml|properties)= などを配置して [[https://docs.spring.io/spring-boot/docs/3.0.1/reference/htmlsingle/#features.external-config.files.profile-specific][プロファイル固有の設定を記述]] するのを基本として、
[[https://docs.spring.io/spring-boot/docs/3.0.1/reference/htmlsingle/#features.external-config.files.importing][=spring.config.import=]] で追加の設定ファイルを読み込んだり、
[[https://docs.spring.io/spring-boot/docs/3.0.1/reference/htmlsingle/#features.external-config.files.multi-document][YAML のマルチドキュメント]] と =spring.config.activate.on-profile= などを使って [[https://docs.spring.io/spring-boot/docs/3.0.1/reference/htmlsingle/#features.external-config.files.activation-properties][特定の条件で Property を有効化]] したり、
[[https://docs.spring.io/spring-boot/docs/3.0.1/reference/htmlsingle/#features.profiles.groups][Profile Groups]] でプロファイルをグループ化したり、等々。
...
実際のテストコードはこちら( [[https://github.com/yewton/asobiba/blob/8af5b30f3c66669b1d83988713b98e6d32ef6aff/reactive-webapp/src/test/kotlin/net/yewton/asobiba/reactivewebapp/ProductionConfigTest.kt][ProductionConfigTest.kt]] )。
[[https://kotest.io/][Kotest]] での記述になっているが、 Kotest 固有の部分は無い。
...
[[https://docs.spring.io/spring-framework/docs/6.0.3/javadoc-api/org/springframework/test/context/ContextConfiguration.html][=@ContextConfiguration=]] に [[https://docs.spring.io/spring-boot/docs/3.0.1/api/org/springframework/boot/test/context/ConfigDataApplicationContextInitializer.html][=org.springframework.boot.test.context.ConfigDataApplicationContextInitializer=]] を指定することで、 =application.properties= 等の設定ファイルをロードするよう指定出来るのは、冒頭の参考リンクにある通り。

#+begin_src kotlin
@ActiveProfiles("production")
#+end_src

[[https://docs.spring.io/spring-framework/docs/6.0.3/javadoc-api/org/springframework/test/context/ActiveProfiles.html][@ActiveProfiles]] でテストしたいプロファイルを指定する。

#+begin_src kotlin
class ProductionConfigTest(env: ConfigurableEnvironment) : FunSpec({
#+end_src

設定値は [[https://docs.spring.io/spring-framework/docs/6.0.3/javadoc-api/org/springframework/core/env/ConfigurableEnvironment.html][=ConfigurableEnvironment=]] を autowire して取得する。

{{{begin_callout(info)}}}
=ConfigurableEnvironment= は、 内部的に複数の [[https://docs.spring.io/spring-framework/docs/6.0.3/javadoc-api/org/springframework/core/env/PropertySource.html][=PropertySource=]] を保持しており、これらを使ってプロパティの値を解決する。

試しにデバッガで見てみると、読み込まれているファイル群を確認出来る:
#+attr_org: :width 300px
[[file:images/2022-12-29_spring-boot-config-test/2022-12-29_23-31-26_screenshot.png]]

ちなみに各 =PropertySource#source= に、 =Map= として設定値が格納されている:
#+attr_org: :width 300px
[[file:images/2022-12-29_spring-boot-config-test/2022-12-29_23-48-03_screenshot.png]]
...
[[file:images/2022-12-29_spring-boot-config-test/2022-12-29_23-31-26_screenshot.png][=PropertyResolver#getProperty=]] メソッドで、特定のプロパティの値を取得出来る。

この値を利用してどういったテストを書くかは、状況に応じて。

** DONE タスクランナー Task の紹介スライドを書いた :task:taskfile:make:
CLOSED: [2022-11-24 木 22:04]
:PROPERTIES:
:EXPORT_FILE_NAME:   index
:EXPORT_DATE:        2022-11-23
:DIR:                images/2022-11-23_task
:EXPORT_HUGO_BUNDLE: 2022-11-23_task
:EXPORT_HUGO_SLUG:   task
:END:
...
[[https://amzn.to/3TSpkqM][Spring Microservices in Action, Second Edition]] の独習用に、 https://github.com/yewton/my-smia というリポジトリで、実際に AWS に Terraform で環境構築してデプロイするところまでをやっている。

その際に利用したタスクランナー [[https://taskfile.dev/][Task]] が、丁度いい感じに使えて良かった為、紹介するためのスライドを作った:

@@markdown:{{< icon name="external-link-alt" pack="fas" >}}@@ [[https://yewton.github.io/my-marp-slides/2022-11_task/PITCHME.html][タスクランナー Task の紹介]]

スライドにも書いたが、以下の点が特に気に入っている:

1. 環境変数の設定・切り替えが簡単
2. [[https://pkg.go.dev/text/template][Go Template]] が使える
3. タスクの構造化が簡単・直感的
4. 無駄な実行を防ぐのが簡単
5. [[https://github.com/SchemaStore/schemastore/blob/master/src/schemas/json/taskfile.json][JSON スキーマが公開されており]] 入力支援が効く

その他のツールとして YAML テンプレーティングツール [[https://carvel.dev/ytt/docs/latest/][ytt]] も便利だった。
こちらについてもその内紹介したい。
...
* Emacs                                                              :@Emacs:
** DONE Org Mode でリンク種別に応じたプレフィクスを表示する
CLOSED: [2023-04-01 土 22:47]
:PROPERTIES:
:EXPORT_FILE_NAME:   index
:EXPORT_DATE:        2023-04-01
:DIR:                images/2023-04-01_org-link-prefix
:EXPORT_HUGO_BUNDLE: 2023-04-01_org-link-prefix
:EXPORT_HUGO_SLUG:   org-link-prefix
:EXPORT_HUGO_TAGS: org-mode emacs org-link emoji
:END:
...
[[https://github.com/awth13/org-appear][org-appear]] でカーソルを合わせた時に全体を表示させたり、
...
[[https://github.com/yewton/.emacs.d/blob/86b2642b1210f1c43489aa6b1e12d1c70856f8df/lisp/toncs-config.org#%E3%83%95%E3%82%A9%E3%83%B3%E3%83%88][この設定]] 等を参考にすると、絵文字に対応したフォントセット設定が可能。
...
[[https://www.gnu.org/software/emacs/manual/html_node/elisp/Lexical-Binding.html][Lexical Binding]] が有効でないとエラーになります。
{{{end_callout}}}
